...
| 2026-07-07 | Post-MVP | ✅ Complete | Design audit + cleanup, on branch `design-audit-cleanup`. Ethan asked for a full whole-app `impeccable:impeccable` design audit (not scoped to one file) checking specifically for AI slop, then a branch to fix what it found — the first real use of the git-branch workflow since the "revisit once all 10 sprints are done" trigger point. Ran the plugin's deterministic `detect.mjs` scanner across all of `src/` and did a manual pass against the plugin's Absolute Bans list. Verdict: mostly clean (no gradient text, no glassmorphism, no hero-metric template, no card-grid sameness, no numbered-eyebrow scaffolding), but three real findings: (1) emoji used as UI icons/copy flourish, inconsistent with the rest of the app's all-lucide icon vocabulary; (2) a genuine, named Absolute Ban hit — `Timeline.css`'s `.timeline-note` had a `border-left: 3px solid green` side-stripe accent; (3) ~70 real em-dash instances in rendered copy (`tourSteps.js`, `whyDmHireFeatures.js`, and most views), which reads as a distinct LLM-writing tell once you notice the density. Fixed all three: swapped all 28 Why DM Hire emoji icons for lucide components via a new `icon` key + `ICON_MAP` in `WhyDMHire.jsx` (kept `whyDmHireFeatures.js` framework-agnostic, matching every other file in `data/` — no React imports there) — note `Linkedin` isn't exported by the installed lucide-react version (brand icons were dropped), used `Users` instead; removed every other emoji/unicode-symbol flourish (👋 🎉 ✦ ✓ ⚠ 📝) across Dashboard/Offers/Pipeline/CandidatePortal, replacing with existing lucide icon components where the symbol was doing an icon's job, or plain text where it was pure decoration; rewrote all ~70 em-dash instances with contextually appropriate punctuation (period, colon, comma) rather than a blind find-replace — left the handful of single-em-dash "empty cell" placeholders (`?? '—'` in Offers/Reports/Settings/CandidateProfile) as a separate, legitimate UI convention but swapped them to plain hyphens anyway per Ethan's literal "any em-dash" ask; removed the Timeline side-stripe entirely (the background tint already does the separation job, per the ban's own suggested fix). Also closed the systemic gap the color-advisory noise pointed at all session: added a "Status & Category Badges" section to `DESIGN.md` documenting the badge-color ramp that's been consistently reused since Sprint 0b but never written down, plus the actual hex values to the frontmatter `colors:` map so the detector recognizes them going forward — re-ran `detect.mjs` after and `design-system-color` findings dropped from 65 to 25 (remaining ones are other one-off colors already individually reviewed as legitimate reuse earlier in the session, not new drift). Verified with `npm run lint` + `npm run build` after every fix. **This work is on `design-audit-cleanup`, pushed but not merged** — first real exercise of the resumed branch workflow; Ethan pulls/merges to `main` on his own schedule. **Update, same day: merged to `main`** at Ethan's request ("go ahead and pull to main") — clean merge, verified with `npm run lint`+`npm run build` on `main` after, branch left on origin (not deleted, wasn't asked). |
| 2026-07-07 | Post-MVP | ✅ Complete | Design system rebrand to Doeren Mayhew's real corporate identity, on branch `design-system-rebrand` (experimental, not merged). Immediately after the audit-cleanup merge, Ethan asked to verify the just-shipped changes visually — this was the first session where Playwright actually worked (Chrome got installed locally, resolving the `sudo`/no-Chrome blocker every prior sprint hit) — confirming the emoji-to-icon swaps, the Timeline side-stripe removal, and the Sprint 10 modal/sidebar-collapse fixes all render correctly, not just compile. Then Ethan shared Doeren Mayhew's actual "Employee Branding Guidelines" PDF and asked for a comparison against DM Hire's invented system; that comparison (an artifact, since it's mostly color swatches and type samples) showed the gap was total: DM Hire's navy/green/Inter system has zero relationship to DM's real identity (Maroon `#6D174E`/PMS 2357, Orange `#F26F2D`/PMS 3564, Cyan `#28A6BC`/PMS 6128 + Teal `#145571`/PMS 7700, paired with Mackinac serif + Tenon sans typefaces — DM's own guide names Libre Baskerville + Archivo as the Google Fonts substitutes for on-screen use). Ethan's call: "let's start with making it match the DM corporate identity," full latitude on execution, explicitly OK with the branch not merging if it doesn't work out. Executed as a token-layer rename rather than touching every component by hand: `sed`-renamed `--color-navy*` → `--color-maroon*` and `--color-green*` → `--color-cyan*` across all 28 files that referenced them (CSS custom properties + the handful of JS files using the same var names inline), then set the actual new hex values once in `tokens.css`. Cyan absorbed the generic `success` semantic too, so there's exactly one positive color in the app now, not two. Typography: swapped the Inter Google Fonts import for Libre Baskerville + Archivo, added a `--font-family-display` token, and applied it to exactly one spot per screen (`.page-title`, the Dashboard's greeting `h1`, the sidebar wordmark) — matching DM's own "Mixing Typefaces" rule that the serif marks only the single most-prominent headline per deliverable, never dense UI. Caught and fixed three things the mechanical rename couldn't reach: (1) hardcoded `rgba()` literals using the old navy/green/orange RGB triples as tint backgrounds (`rgba(13,45,92,...)`, `rgba(109,179,63,...)`, `rgba(229,84,28,...)`) across 8 files, invisible to a `var()`-only find-replace; (2) the Pipeline funnel's ordinal ramp (`PipelineFunnel.jsx`), which was a hardcoded 5-step green sequential scale from the Sprint 1 dataviz work, completely independent of the token system — rebuilt as a light-pale-cyan-to-dark-teal ramp with contrast hand-verified at the ink crossover point (maroon-ink steps ≥4.7:1, white-ink steps ≥4.8:1); (3) the offer panel's gradient (`CandidateProfile.css`'s `.cp-offer-panel`), where translucent-white label text and a cyan accent had been contrast-tuned in Sprint 10 specifically against navy's much-darker light-end (`navy-light`) — recomputed by hand against the new, notably lighter `maroon-light` and found the existing `.65` opacity had dropped to ~4.1:1, below the 4.5:1 AA floor; bumped label opacities to `.75` and swapped the accent to a purpose-lightened cyan tint (`#A0E0EA`) rather than reusing `--color-cyan-light` (which is tuned for other, darker contexts). Rewrote `DESIGN.md` as the actual source of truth rather than leaving it stale: new Colors/Typography prose, updated frontmatter keys and component refs, and a new Section 6 "Logo" documenting that the sidebar's plain-text wordmark is an explicit placeholder — the branding PDF only contains embedded slide-deck preview images, not usable production logo files, so the real Doeren Mayhew logo can't go in the app until DM provides actual PNG/SVG/AI assets; that's a hard external blocker, not something to approximate from the PDF. Verified visually via Playwright screenshots across Dashboard/Pipeline/Candidate Profile/Why DM Hire/Offers (not just lint+build) before committing — this is the first sprint-adjacent work done with working browser verification. Pushed to `design-system-rebrand`, deliberately not merged — Ethan's explicit "if we hate it we don't have to merge with main." |
